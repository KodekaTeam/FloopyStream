...
function updateTablePagination() {
  ...
  if (nextBtn)
    nextBtn.disabled = currentTablePage === totalPages || totalPages <= 1;
}
...
function filterTableRows() {
  ...
  console.log(
    "filterTableRows: filteredBroadcasts.length:",
    filteredBroadcasts.length
  );
  ...
  allRows.forEach((row, rowIndex) => {
    ...
    const isInFilteredList = filteredBroadcasts.some(
      (b) => b.id === broadcastId
    );

    if (rowIndex < 3) {
      // Log first 3 rows only
      console.log(
        `Row ${rowIndex}: ID=${broadcastId}, inFiltered=${isInFilteredList}, filteredRowIndex=${filteredRowIndex}`
      );
    }
    ...
    if (filteredRowIndex >= start && filteredRowIndex < end) {
      ...
      if (rowIndex < 3) {
        console.log(
          `  → SHOWING (${filteredRowIndex} in range ${start}-${end})`
        );
      }
    } else {
      row.style.display = "none";
      if (rowIndex < 3) {
        console.log(
          `  → HIDING (${filteredRowIndex} outside range ${start}-${end})`
        );
      }
    }
...
function closeNewStreamModal() {
  ...
  document.getElementById("destinationUrlInput").placeholder =
    "Enter custom RTMP URL";
}
...
function selectVideoFromDropdownItem(element) {
  ...
  document
    .getElementById("selectedVideoText")
  ...
  if (type === "playlist") {
    ...
    if (videoInfo) {
      document.getElementById("streamVideoTitle").textContent = title;
      document.getElementById("streamVideoResolution").textContent =
        resolution || "N/A";
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        showNotification(
          result.message || "Failed to create broadcast",
          "error"
        );
      }
    } catch (error) {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (errorMsg.includes("Facebook connection error")) {
        showNotification(
          "⚠️ Facebook connection busy. Please wait 10-15 seconds and try again.",
          "error"
        );
      } else {
...
async function stopBroadcast(broadcastId) {
  ...
  try {
    ...
    if (result.success) {
      showNotification(
        "Broadcast stopped. Wait 10-15 seconds before restarting (especially for Facebook).",
        "success"
      );
...
async function updateSystemStats() {
  try {
    ...
    if (result.success) {
      ...
      // Update Memory
      const memoryUsedGB = (stats.memory.used / (1024 * 1024 * 1024)).toFixed(
        2
      );
      const memoryTotalGB = (stats.memory.total / (1024 * 1024 * 1024)).toFixed(
        2
      );
...
async function openStreamInfoModal(broadcastId) {
  try {
    ...
    if (data.success) {
      ...
      document.getElementById("infoStreamName").textContent =
        broadcast.broadcast_name || "N/A";
      document.getElementById("infoPlatform").textContent =
        broadcast.platform_name || "N/A";
      document.getElementById("infoStatus").textContent =
        broadcast.broadcast_status || "N/A";
      ...
      document.getElementById("infoDestination").textContent =
        broadcast.destination_url || "N/A";
      document.getElementById("infoStreamKey").textContent =
        broadcast.stream_key || "N/A";
...
async function openEditStreamModal(broadcastId) {
  try {
    ...
    if (data.success) {
      ...
      document.getElementById("editStreamName").value =
        broadcast.broadcast_name || "";
      document.getElementById("editDestinationUrl").value =
        broadcast.destination_url || "";
      document.getElementById("editStreamKey").value =
      ...
      document.getElementById("editBroadcastIdDisplay").textContent =
        broadcast.broadcast_id;

      // Content type
      const contentType =
        broadcast.content_type === "playlist" ? "Playlist" : "Single Video";
      ...
      const statusColor =
        statusColors[broadcast.broadcast_status] || "text-gray-400";
      statusElement.innerHTML = `
        <span class="inline-flex items-center gap-1 px-2 py-1 bg-gray-700 rounded text-xs">
          ...
          <span class="capitalize">${
            broadcast.broadcast_status || "Unknown"
          }</span>
        </span>
      `;

      // Show/hide Schedule Settings preview
      const scheduleSettingsPreview = document.getElementById(
        "editScheduleSettingsPreview"
      );
      if (scheduleSettingsPreview) {
        ...
        if (hasSchedule || loopVideo || hasDurationTimeout) {
          ...
          document.getElementById("editPreviewLoopVideo").textContent =
            loopVideo ? "Yes" : "No";
          document.getElementById("editPreviewScheduleTime").textContent =
      ...
      const advancedSettingsPreview = document.getElementById(
        "editAdvancedSettingsPreview"
      );
