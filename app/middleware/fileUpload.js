...
const thumbnailStorage = multer.diskStorage({
  destination: (req, file, callback) => {
    const uploadDir = process.env.THUMBNAIL_DIR || './storage/thumbnails';
    try {
  ...
  filename: (req, file, callback) => {
    const uniqueId = uuidv4();
    const extension = path.extname(file.originalname);
    const filename = `thumb_${uniqueId}${extension}`;
    callback(null, filename);
  }
});
...
module.exports = {
  ...
  thumbnailUploader: thumbnailUploader.single('thumbnail'),
  handleUploadError
};
