...
router.post('/start', requireAuth, async (req, res) => {
  try {
    const { contentId, platformName, destinationUrl, streamKey, scheduledTime, broadcastName, loopVideo, duration, bitrate, framerate, resolution, orientation } = req.body;
    ...
    if (!contentId || !destinationUrl) {
      ...
      return res.status(400).json({ 
        success: false, 
        message: 'Missing required fields (contentId and destinationUrl are required)' 
      });
    }
    ...
    const actualContentId = playlist ? playlist.playlist_id : content.content_id;
    ...
    const broadcastData = {
      ...
      broadcastName: (broadcastName && broadcastName.trim() !== '') ? broadcastName.trim() : contentName,
    ...
    const result = await Broadcast.createNew(req.session.accountId, broadcastData);
    ...
    res.json({ 
      success: true, 
      message: scheduledTime ? 'Broadcast scheduled successfully' : 'Broadcast created successfully. Click "Start" to begin streaming.',
      broadcastId: result.broadcastId
    });
  } catch (error) {
...
router.post('/start/:broadcastId', requireAuth, async (req, res) => {
  try {
    ...
    // Check ownership
    if (broadcast.account_id !== req.session.accountId && req.session.accountRole !== 'admin') {
      return res.status(403).json({ success: false, message: 'Access denied' });
    }

    // Check if broadcast is already active
    if (broadcast.broadcast_status === 'active') {
      return res.status(400).json({ success: false, message: 'Broadcast is already active' });
    }
    ...
    if (broadcast.content_type === 'playlist') {
      ...
      if (!playlist || !playlist.videos || playlist.videos.length === 0) {
        return res.status(404).json({ success: false, message: 'Playlist is empty or not found' });
      }
      ...
      res.json({ 
        success: true, 
        message: `Playlist broadcast started with ${playlist.videos.length} videos (${isShuffleMode ? 'Shuffle' : 'Sequential'} mode)`
      });
    } else {
      ...
      await startLiveBroadcast(parseInt(broadcastId), videoPath, broadcast.destination_url, broadcast.stream_key, null, {
        bitrate: broadcast.bitrate,
        frame_rate: broadcast.frame_rate,
        resolution: broadcast.resolution,
        orientation: broadcast.orientation
    ...
    await logError('Failed to start broadcast manually', { error: error.message });
    res.status(500).json({ success: false, message: error.message });
  }
});
...
router.put('/:broadcastId', requireAuth, async (req, res) => {
  try {
    ...
    console.log('Update broadcast request:', { broadcastId, broadcast_name, destination_url, stream_key });
    ...
    if (!broadcast_name || !destination_url) {
      return res.status(400).json({ success: false, error: 'Broadcast name and destination URL are required' });
    }
...
router.delete('/:broadcastId', requireAuth, async (req, res) => {
  try {
    ...
    // Delete record
    await executeQuery('DELETE FROM broadcasts WHERE broadcast_id = ?', [broadcastId]);
