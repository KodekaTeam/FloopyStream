...
router.get("/:contentId", requireAuth, async (req, res) => {
  try {
    ...
    // Check ownership
    if (
      content.account_id !== req.session.accountId &&
...
router.delete("/:contentId", requireAuth, async (req, res) => {
  try {
    ...
    if (relatedBroadcasts && relatedBroadcasts.length > 0) {
      const broadcastList = relatedBroadcasts
        .map((b) => {
      ...
      console.log(
        "Delete blocked. Broadcasts using this video:",
        broadcastList
      );
  ...
  try {
    ...
    const formattedContent = contentList.map((content) => {
      const duration = content.duration_seconds
        ? Math.floor(content.duration_seconds)
        : 0;
...
router.post("/drive/import-url", requireAuth, async (req, res) => {
  try {
    ...
    if (!driveUrl) {
      return res
        .status(400)
        .json({ success: false, message: "Google Drive URL is required" });
    }

    const {
      extractFileId,
      downloadFile,
    } = require("../../utilities/gdriveDownloader");
